  ...
  data () {
    return {
      data: {
        ...
        rotateSensitivity: 1, // 旋转操作的灵敏度，值越大越灵敏。支持使用数组分别设置横向和纵向的旋转灵敏度。0不能旋转
  ...
  methods: {
    processData (data) {
      ...
      $.each(data, function (i, p) {
        if (i === 0) { that.data.max = p.tcc }
        if (i === (data.length - 1)) { that.data.min = p.tcc }
        var dataRegion = {
          name: p.cname,
          value: [p.tcc, p.tvus, (i + 1)]
        }
        that.data.regionData.push(dataRegion)
        ...
        if (fromToNameCoords && i <= 4) { // 识别不了经度信息的必须剔除  不然会报错  痛过
          linesdata1 = {
            fromName: '某海关',
            toName: p.cname,
            coords: [that.data.geoCoord['某海关'], fromToNameCoords],
            value: [p.tcc, p.tvus, (i + 1)]
          }
          that.data.hg_zt_data_lines1_top5.push(linesdata1)
          linesdata2 = {
            fromName: p.cname,
            toName: '某海关',
            coords: [fromToNameCoords, that.data.geoCoord['某海关']],
            value: [p.tcc, p.tvus, (i + 1)]
          }
          that.data.hg_zt_data_lines2_top5.push(linesdata2)
          pointdata = {
            name: p.cname,
            value: fromToNameCoords,
            pc: p.tcc,
            hz: p.tvus,
            pm: (i + 1)
          }
          that.data.point_data_crj.push(pointdata)
        } // else{console.log(p.cname);}
        // 后十五名
        if (fromToNameCoords && i > 4 && i < 20) { // 识别不了经度信息的必须剔除  不然会报错  痛过
          linesdata1 = {
            fromName: '某海关',
            toName: p.cname,
            coords: [that.data.geoCoord['某海关'], fromToNameCoords],
          ...
          that.data.hg_zt_data_lines1.push(linesdata1)
          linesdata2 = {
            fromName: p.cname,
            toName: '某海关',
            coords: [fromToNameCoords, that.data.geoCoord['某海关']],
          ...
          that.data.hg_zt_data_lines2.push(linesdata2)
          pointdata = {
            name: p.cname,
            value: fromToNameCoords,
            pc: p.tcc,
      ...
      this.data.option = {
        ...
        geo3D: {
          ...
          viewControl: {
            ...
            distance: this.data.distance, // 地图大小   值越大-地图越小，值越小-地图越大
            alpha: this.data.alpha, // 垂直翻转角度
            beta: this.data.beta,
            zoomSensitivity: this.data.zoomSensitivity,
            center: this.data.center,
            autoRotateDirection: this.data.autoRotateDirection,
            autoRotateSpeed: this.data.autoRotateSpeed,
            autoRotateAfterStill: this.data.autoRotateAfterStill,
            damping: this.data.damping,
            rotateSensitivity: this.data.rotateSensitivity
          }
    ...
    echartsInit () {
      ...
      this.processData(this.data.hg_zt_json)
      this.makeOption()
